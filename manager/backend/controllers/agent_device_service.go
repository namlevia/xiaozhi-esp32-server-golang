...
func targetUserIDFromScope(scope accessScope, requested uint) (uint, error) {
	if scope.IsAdmin {
		if requested == 0 {
			return 0, fmt.Errorf("Vui lòng chọn người dùng sở hữu")
		}
...
func (svc *AgentService) Get(scope accessScope, id uint) (*AgentResponse, error) {
	...
	if err := query.First(&agent).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, fmt.Errorf("Trợ lý không tồn tại")
		}
		return nil, err
	}
...
func (svc *AgentService) Create(scope accessScope, payload AgentPayload) (*AgentResponse, error) {
	...
	if name == "" {
		return nil, fmt.Errorf("Vui lòng nhập tên trợ lý")
	}
	if len([]rune(name)) > 50 {
		return nil, fmt.Errorf("Tên trợ lý không được vượt quá 50 ký tự")
	}
	if nickname == "" {
		nickname = name
	}
	if len([]rune(nickname)) > 50 {
		return nil, fmt.Errorf("Biệt danh trợ lý không được vượt quá 50 ký tự")
	}
...
func (svc *AgentService) Delete(scope accessScope, id uint) error {
	...
	if err != nil {
		return fmt.Errorf("Truy vấn thiết bị đã liên kết với trợ lý thất bại")
	}
	if deviceCount > 0 {
		return fmt.Errorf("Trợ lý đã liên kết thiết bị, vui lòng gỡ toàn bộ thiết bị trước khi xóa")
	}
...
func (svc *AgentService) validateKnowledgeBaseOwnership(userID uint, knowledgeBaseIDs []uint) error {
	...
	if count != int64(len(uniqueIDs)) {
		return fmt.Errorf("Có ID kho tri thức không hợp lệ hoặc vượt quyền")
	}
	return nil
}
...
func (svc *DeviceService) Create(scope accessScope, payload DevicePayload) (*DeviceResponse, error) {
	...
	if scope.IsAdmin {
		if deviceCode == "" && deviceName == "" {
			return nil, fmt.Errorf("Vui lòng nhập ít nhất một trong mã kích hoạt hoặc định danh thiết bị")
		}
	} else if deviceName == "" {
		return nil, fmt.Errorf("Vui lòng nhập định danh thiết bị")
	}
	if nickName == "" {
		nickName = deviceName
	}
...
func (svc *DeviceService) Update(scope accessScope, id uint, payload DevicePayload) (*DeviceResponse, error) {
	...
	if err := query.First(&device).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, fmt.Errorf("Thiết bị không tồn tại hoặc không thuộc người dùng hiện tại")
		}
		return nil, err
	}
...
func (svc *DeviceService) Delete(scope accessScope, id uint) error {
	...
	if err := query.First(&device).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return fmt.Errorf("Thiết bị không tồn tại hoặc không thuộc người dùng hiện tại")
		}
		return err
	}
...
func (svc *DeviceService) BindToAgent(scope accessScope, agentID uint, payload DevicePayload) (*DeviceResponse, error) {
	...
	if code == "" && deviceName == "" {
		return nil, fmt.Errorf("Vui lòng nhập mã xác minh thiết bị hoặc MAC thiết bị")
	}
	if code != "" && !isSixDigitCode(code) {
		return nil, fmt.Errorf("Định dạng mã xác minh không đúng")
	}
	nickName, err := normalizeDeviceNickName(payload.NickName)
	if err != nil {
		return nil, err
	...
	if deviceExists {
		if device.UserID != 0 && device.UserID != agent.UserID {
			...
			return nil, fmt.Errorf("MAC thiết bị không hợp lệ hoặc thiết bị đã được liên kết")
		}
...
func (svc *DeviceService) UnbindFromAgent(scope accessScope, agentID uint, deviceID uint) error {
	...
	if err := svc.DB.Where("id = ? AND user_id = ? AND agent_id = ?", deviceID, agent.UserID, agent.ID).First(&device).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return fmt.Errorf("Thiết bị không tồn tại hoặc không thuộc trợ lý này")
		}
		return err
	}
...
func (svc *DeviceService) assertAgentOwnedByUser(agentID uint, userID uint) error {
	...
	if count == 0 {
		return fmt.Errorf("Trợ lý không tồn tại hoặc không thuộc người dùng được chỉ định")
	}
	return nil
}
...
func writeServiceError(c *gin.Context, err error, fallback string) {
	...
	if strings.Contains(msg, "không có quyền") || strings.Contains(msg, "vượt quyền") {
...
func getVoiceOptionsForUser(db *gorm.DB, c *gin.Context, targetUserID uint, provider, configID, overrideURL, overrideAPIKey string) ([]VoiceOption, error) {
	...
	if provider == "" {
		return nil, fmt.Errorf("Bắt buộc nhập tham số provider")
	}
	...
	if provider == "indextts_vllm" {
		...
		if err != nil {
			return nil, fmt.Errorf("Lấy giọng IndexTTS thất bại: %w", err)
		}
		systemVoices = voices
	} else if provider == "piper" {
		uc := &UserController{DB: db}
		voices, err := uc.fetchPiperVoices(c, configID, overrideURL)
		if err != nil {
			return nil, fmt.Errorf("Lấy giọng Piper thất bại: %w", err)
		}
		systemVoices = voices
