...
func NewCozeLLMProvider(config map[string]interface{}) (*CozeLLMProvider, error) {
	...
	if apiKey == "" {
		return nil, fmt.Errorf("coze api_key không được rỗng")
	}

	botID, _ := config["bot_id"].(string)
	botID = strings.TrimSpace(botID)
	if botID == "" {
		return nil, fmt.Errorf("coze bot_id không được rỗng")
	}
...
func (p *CozeLLMProvider) ResponseWithContext(ctx context.Context, sessionID string, dialogue []*schema.Message, _ []*schema.ToolInfo) chan *schema.Message {
	...
	go func() {
		...
		if strings.TrimSpace(query) == "" {
			sendLLMError(out, fmt.Errorf("coze query không được rỗng"))
			return
		}
		...
		for i, currentBody := range requestBodies {
			...
			if i == 0 && len(requestBodies) > 1 {
				log.Warnf("Request Coze đầu tiên thất bại, thử fallback retry: %v", openErr)
			}
		}
		if openErr != nil {
		...
		for event, eventErr := range sse.Read(resp.Body, nil) {
			if eventErr != nil {
				...
				if seenDelta && strings.Contains(strings.ToLower(eventErr.Error()), "unexpected end of input") {
					// Một số instance Coze ngắt kết nối trực tiếp sau event cuối, chấp nhận trường hợp này.
					return
				}
				sendLLMError(out, fmt.Errorf("Đọc stream Coze thất bại: %w", eventErr))
				return
			}
...
func (p *CozeLLMProvider) openStreamRequest(ctx context.Context, bodyBytes []byte) (*http.Response, error) {
	...
	if !strings.Contains(contentType, "text/event-stream") {
		...
		return nil, fmt.Errorf(
			"Response Coze không phải stream SSE path=%s content_type=%s body=%s",
			streamCreatePath,
			contentType,
...
func buildCozeQuery(dialogue []*schema.Message) string {
	...
	// Ở mode session Coze, chỉ gửi input user lượt hiện tại, không nối lịch sử local.
	...
	// Fallback: nếu không có message user, fallback về message cuối cùng có thể trích text.
	for i := len(dialogue) - 1; i >= 0; i-- {
	...
	if payload, ok := parseJSONMap(data); ok {
		...
		if nestedData, ok := payload["data"]; ok {
			...
			case string:
				normalized := normalizeCozeStreamData(v)
				if normalized != "" && normalized != data {
					if nestedMsg := extractCozeError(cozeStreamEvent{}, normalized); nestedMsg != "coze trả lỗi" {
						return nestedMsg
					}
				}
			case map[string]any:
...
func (p *CozeLLMProvider) ResponseWithVllm(_ context.Context, _ []byte, _ string, _ string) (string, error) {
	return "", fmt.Errorf("Provider coze không hỗ trợ năng lực vllm")
}
